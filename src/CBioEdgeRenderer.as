...
package org.cytoscapeweb.view.render {
	...
	public class CBioEdgeRenderer extends EdgeRenderer {
        ...
        private function drawArrow(g:Graphics, start:Point, end:Point, eb1:Point, eb2:Point,
			...
			// SBGN modificaiton !
			// Find perpendicular vector points for the arrow base:
			var n1:Point = new Point(end.x, end.y);
			var n2:Point = n1.clone();
			n1.normalize(1);
			n2.normalize(-1);
			var t1:Point = new Point(n1.y, -n1.x);
			var t2:Point = new Point(n2.y, -n2.x);
			t1 = new Point(start.x + t1.x * aw, start.y + t1.y * aw);
			t2 = new Point(start.x + t2.x * aw, start.y + t2.y * aw);
            ...
            switch (shape)
			{
				//SBGN Modification for Edge Renderer
				case "T-ARROW":
					...
					// Find additional line's points for necessary stimulation class edges
					var c5:Point = new Point(2*b1.x-(c1.x+b1.x)/2, 2*b1.y-(c1.y+b1.y)/2 );
					var c6:Point = new Point(2*b2.x-(c2.x+b2.x)/2, 2*b2.y-(c2.y+b2.y)/2 );
